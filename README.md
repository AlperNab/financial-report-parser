...
**Domain:** `Finance / Equity Research` · **Suite:** `Finance Document Suite` · **Accent:** `#67e8f9`
...
**Financial Report Parser** is a standalone, browser-based AI workflow app for **Finance / Equity Research**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
|---|---:|:---:|---|---|
| `annual`<br>Annual | text | Yes | — | Affects input: Annual. |
| `earnings_reports`<br>earnings reports | text | Yes | — | Affects input: earnings reports. |
...
| `company_type`<br>company type | text | No | — | Affects customization: company type. |
| `currency`<br>currency | select | No | USD | Affects customization: currency. |
| `fiscal_year`<br>fiscal year | text | No | — | Affects customization: fiscal year. |
| `accounting_standard`<br>accounting standard | text | No | — | Affects customization: accounting standard. |
| `metrics_list`<br>metrics list | text | No | — | Affects customization: metrics list. |
...
uvicorn app.main:app --host 127.0.0.1 --port 9122
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9122`. |
...
```bash
curl http://127.0.0.1:9122/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9122/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9122 uvicorn app.main:app --host 0.0.0.0 --port 9122
```
...
- `plugins/financial-report-parser.json` for plugin metadata.
